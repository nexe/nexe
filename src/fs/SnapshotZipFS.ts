...
import {CreateReadStreamOptions, CreateWriteStreamOptions, BasePortableFakeFS, ExtractHintOptions} from '@yarnpkg/fslib/lib/FakeFS';
...
export class SnapshotZipFS extends BasePortableFakeFS {
  static async openPromise<T>(fn: (zipOpenFs: SnapshotZipFS) => Promise<T>, opts: ZipOpenFSOptions): Promise<T> {
  ...
  constructor({libzip, baseFs = new NodeFS(), filter = null, maxOpenFiles = Infinity, readOnlyArchives = false, useCache = true, mountAt, zipFs}: ZipOpenFSOptions) {
    super();
